...
class Basic(Player):
    ...
    def _play(self, players, game):
        ...
        known_cards = [card for card in self.hand if card.suit(is_visible=False) and card.value(is_visible=False)]

        if known_cards:
            return sorted(known_cards, key=lambda c: c.value(is_visible=False))[0]
        else:
            return None
    ...
    def _discard(self, players, game):
        ...
        for card in self.hand:
            for suit, pile in game.piles.items():
                if card.suit(is_visible=False) and \
                   card.value(is_visible=False) and \
                   card in pile:
                    return card
                elif (card.suit(is_visible=False)==suit or \
                     (card.suit(is_visible=False)=='Rainbow' and ~game.rainbow_as_sixth)) and \
                     not card.value(is_visible=False) and \
                     game._is_suit_complete(pile):
                    return card
                elif not card.suit(is_visible=False) and \
                     card.value(is_visible=False) and \
                     pile and card.value(is_visible=False) <= pile[-1].value():
                    return card
                elif not card.suit(is_visible=False) and \
                     not card.value(is_visible=False):
                    return card
        
        # discard the lowest valued card in hand (assigning '6' to those whose value is not known),
        # if no other criteria has been met
        return sorted(
            self.hand, key=lambda c: c.value(is_visible=False) \
            if c.value(is_visible=False) is not None \
            else 6
        )[0]
    ...
    def _arrange_hand(self):
        ...
        known_cards = [card for card in self.hand
        ...
        suit_only_cards = [card for card in self.hand
                           if card.suit(is_visible=False) and not card.value(is_visible=False)
        ...
        value_only_cards = [card for card in self.hand
                            if not card.suit(is_visible=False) and card.value(is_visible=False)
                                                               and card not in arranged_hand]
        ...
        arranged_hand += [card for card in self.hand if card not in arranged_hand]
